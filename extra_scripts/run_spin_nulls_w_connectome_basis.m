...
load_data = load(sprintf('./osf_dl/results/basis_connectome_midthickness-%s_evec_%i.mat', hemisphere, num_modes));

eigenmodes = load_data.eig_vec ; 
...
for map_idx = 1:length(map_names)
    ...
    filename = sprintf('./gen_data/spinres_connectome_%s_%s-%s.mat',map_names{map_idx},surface_interest,hemisphere) ; 
